...
namespace sdlw{
    ...
    void FileManager::setPrefPath(const char* company_name, const char* app_name){
        ...
        if(!pref_path)
            error("cannot set prefpath as %s, %s", company_name, app_name);
    }
    ...
    int FileManager::load_pref(const char* relative_path, void* data){ 
        std::string path = pref_path;
        path += relative_path;
        FILE* f = fopen(path.c_str(), "rb");
        if(f == NULL){
    ...
    */
    ...
    SpriteSheet::SpriteSheet(Window* window, const char* atlas_info_csv) {
        load(window->getRenderer(), atlas_info_csv);
    } 
    int SpriteSheet::load(SDL_Renderer* renderer, const char* atlas_info_csv){
        ...
        while(true){
            ...
            if(path2sprite.find(sprite_name) == path2sprite.end())
            ...
            char sprite_name_last = sprite_name[strlen(sprite_name)-5]; // #.png
            if(sprite_name_last >= '0' && sprite_name_last <= '9'){
                int i;
                for(i=strlen(sprite_name)-6; sprite_name[i] >= '0' && sprite_name[i] <= '9'; --i);
                if(path2sprite_bundle_vector.find(std::string(sprite_name, i+1)) == path2sprite_bundle_vector.end())
                    path2sprite_bundle_vector.insert({std::string(sprite_name, i+1), std::vector<Sprite*>()});
                path2sprite_bundle_vector.at(std::string(sprite_name, i+1)).push_back(path2sprite.at(std::string(sprite_name)));
            }
        }
    ...
    void Window::handleAfterRender(){
        if(afterRenderHandler) afterRenderHandler();
        if(current_scene) current_scene->handleAfterRender();
    }
    ...
    void Input::render(){
        ...
        if(focused == this && ++counter % FPS < FPS*0.5)
    ...
    void MainLoop::run(int fps){
        ...
        auto loop_func = [](void* args){
            ...
            while(SDL_GetTicks() - _this->last_time >= 1000/FPS){
                if(_this->before_draw_handler) _this->before_draw_handler();
                for(Window*& w : _this->windows) w->render(); 
                if(_this->after_draw_handler) _this->after_draw_handler();
                _this->last_time += 1000/FPS;
            }
        };
